...
// ✨ 100% Foolproof Case-Insensitive Search Fix
    if (search && search.trim() !== '') {
      const cleanSearch = `%${search.trim().toLowerCase()}%`;
      query += ` AND (LOWER(t.customer_name) LIKE ? OR CAST(t.id AS TEXT) LIKE ?)`;
      params.push(cleanSearch, cleanSearch);
    }

    // ✨ FIXED: Expanded Sorting options including ID Ascending
    switch (sort) {
      case 'id_asc':
      ...
      case 'newest':
      default:
        query += ` ORDER BY t.id ASC`; // ✨ Defaulted to Ascending order for IDs as requested
        break;
    }
